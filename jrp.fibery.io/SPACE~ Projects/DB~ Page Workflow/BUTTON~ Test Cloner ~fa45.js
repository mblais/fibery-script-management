//.fibery SCRIPTID=64d2a7ff75ac7ced2a80345d ACTIONID=f32b6291-699c-4c9a-bc75-1e601e16fa45
...
// Is an entity a desgnated template entity? I.e. does it have an "Is Template" field that is truthy?
...
const linkRuleFields = (fieldObject) => {
    function getLinkRuleFields() {
        ...
        if (!relatedFieldObject) {
            log(`${debugInfo} - null relatedFieldObject`)
            return null
        }
        const linkRule = fieldObject.linkRule || relatedFieldObject.linkRule
        if (!linkRule) {
            log(`${debugInfo} - null linkRule`)
            return null
        }
...
class EntityProxy {
    ...
    updateReferences(clone) {
        ...
        for (const { entity, fieldName, idx } of entityRefs[templateKey]) {
...
async function make_EntityProxy(type, id, entityPromise, isClone = null) {
    ...
    if (id) setEntity(id)                     // Entity already exists
    const entity = await entityPromise
    ep.setEntity(entity)                        // Now entity definitely exists
    if (!id) setEntity(entity.Id)             // Get Id from created entity
    return ep
}
